...
class User
{
    ...
    public function findOthers($email)
    {
        ...
        $statement->bindValye(":email", $email);
        $statement->execute();
    ...
    public function findPerfectMatch($others)
    {
        ...
        foreach ($others as $other => $value) {
            $conn = Db::getConnection();
            $statement = $conn->prepare("SELECT * FROM users where buddy != :buddy AND music = :music AND movies = :movies AND games = :games AND books = :books");

            $statement->bindValue(":buddy", $other["buddy"]);
            $statement->bindValue(":music", $other["music"]);
            $statement->bindValue(":movies", $other["movies"]);
            $statement->bindValue(":games", $other["games"]);
            $statement->bindValue(":books", $other["books"]);
            $statement->execute();
